...
func TestPolicyPodLevelSingleNumaNodeCanAdmitPodResult(t *testing.T) {
	tcases := []struct {
		name     string
		hint     TopologyHint
		expected bool
	...
	for _, tc := range tcases {
		...
		result := policy.(*podLevelSingleNumaNodePolicy).canAdmitPodResult(&tc.hint)
...
func TestPolicyPodLevelSingleNumaNodeFilterHints(t *testing.T) {
	tcases := []struct {
		name              string
...
func TestPolicyPodLevelSingleNumaNodeMerge(t *testing.T) {
	numaNodes := []int{0, 1}
	policy := NewPodLevelSingleNumaNodePolicy(numaNodes)

	tcases := commonPolicyMergeTestCases(numaNodes)
	tcases = append(tcases, policy.(*podLevelSingleNumaNodePolicy).mergeTestCases(numaNodes)...)
